...
Most notably, the DigitalOcean API Token must be specified.  They
can be specified in the INI file or with the following environment variables:
    export DO_ACCESS_TOKEN='abc123'

Alternatively, it can be passed on the command-line with --access-token.
...
usage: digital_ocean.py [-h] [--list] [--host HOST] [--all]
                                 ...
                                 [--refresh-cache] [--access-token ACCESS_TOKEN]
...
optional arguments:
  ...
  --access-token ACCESS-TOKEN, -a ACCESS_TOKEN
...
class DigitalOceanInventory(object):
    ...
    def __init__(self):
        ...
        if not hasattr(self, 'access_token'):
            print '''Could not find values for DigitalOcean ACCESS token.
It must be specified via either ini file, command line argument (--access-token),
or environment variables (DO_ACCESS_TOKEN=)'''
            sys.exit(-1)

        # env command, show DigitalOcean credentials
        if self.args.env:
            print "DO_CLIENT_ID=%s" % (self.access_token)
            sys.exit(0)
    ...
    def read_settings(self):
        ...
        # Credentials
        if config.has_option('digital_ocean', 'access_token'):
            self.access_token = config.get('digital_ocean', 'access_token')
    ...
    def read_environment(self):
        ...
        # Setup credentials
        if os.getenv("DO_ACCESS_TOKEN"): self.access_token = os.getenv("DO_ACCESS_TOKEN")
    ...
    def read_cli_args(self):
        ...
        parser.add_argument('--env','-e', action='store_true', help='Display DO_ACCESS_TOKEN')
        parser.add_argument('--access-token','-a', action='store', help='DigitalOcean Access Token')

        self.args = parser.parse_args()

        if self.args.access_token: self.access_token = self.args.access_token
    ...
    def build_inventory(self):
        ...
        for droplet in self.data['droplets']:
            ...
            self.push(self.inventory, droplet['region']['slug'], dest)
